...
class LaporanPercetakan extends MainConf
{
		...
		$sqlDetail = "SELECT
			...
			if($arrDetail > 0)
			{
				foreach($arrDetail as $row)
				{
					...
					$tblBody .= '<td>'.$row['jumlah_pesanan'].'</td>';
					$tblBody .= '<td>'.$this->formatCurrency($row['hrg_cetakan'],2).'</td>';
					$tblBody .= '<td>'.$row['est_hari'].'</td>';	
					$tblBody .= '<td>'.$row['tt_hari'].'</td>';	
					$tblBody .= '<td>'.$row['lembur'].' %</td>';		
	...
	public function detailClicked($sender,$param)
	{
		...
		$this->diskonTrans->Text = '<strong>'.$this->formatCurrency($CetakanRecord->diskon,2).'</strong>';
